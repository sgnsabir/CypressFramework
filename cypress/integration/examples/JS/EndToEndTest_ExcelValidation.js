...
describe('Login Bypass', () => {
    it('Logged in through generated token from local storage and end to end validation', async () => {
        ...
        const filePath = projetPath + `\\cypress\\downloads\\order-invoice_${username}.xlsx`
        cy.task('excelToJsonConverter', filePath).then(function (result) {
            //converting json to array
            const dataArray = result.data;
            const productNames = dataArray.slice(1).map(row => row["B"]);
            console.log('product names: ', productNames);
            productList.forEach(product => {
                expect(productNames.includes(product)).to.be.true;
            })
        })
        //to find only one product shortcut but not recommended
